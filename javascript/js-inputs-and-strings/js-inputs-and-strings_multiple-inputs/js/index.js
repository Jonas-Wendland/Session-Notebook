/*

Important: Create a fork for each sub challenge!
Implement the following functionality:

1. On button click: The value of the first input field is copied into the second input field

2. On button click: The value of the first input field is copied in uppercase into the second input field

3. On button click: The values of the two input fields switch.

*/

const firstInput = document.querySelector("[data-js=first-input]");
const secondInput = document.querySelector("[data-js=second-input]");
const buttonFirstToSecond = document.querySelector(
  "[data-js=button-first-to-second]"
);
const buttonFirstToSecondUpper = document.querySelector(
  "[data-js=button-first-to-second-upper]"
);
const buttonSwap = document.querySelector("[data-js=button-swap]");

buttonFirstToSecond.addEventListener("click", () => {
  secondInput.value = secondInput.value + " " + firstInput.value;
});

buttonFirstToSecondUpper.addEventListener("click", () => {
  secondInput.value = secondInput.value + " " + firstInput.value.toUpperCase();
});
buttonSwap.addEventListener("click", () => {
  const swap1 = firstInput.value;
  const swap2 = secondInput.value;

  firstInput.value = swap2;
  secondInput.value = swap1;
});
